...
const { Product, Category, Subcategory, Price } = require("../db")
...
productRoutes.post('/bulkCreate', async (req, res) => {
    const { data } = req.body
    try {
        await data.forEach( async el => {
            try {
                const { categoriesId, stock, price } = el
                let data = { ...el }
                ...
                console.log('ERRORERROEROEOROEROREOROE', err.message)
            }
            
        })
        
        res.status(201).json({sos: 'sis'})
    } catch (err) {
